...
const getPedidos = (request, response) => {
    pool.query("select id, Numero_Phoenix , ProcessoCRM , IndicadorDeInscricaoEstadual , InscricaoEstadual , Cnpj,Nome,Fantasia,Endereco,Numero,Complemento,Bairro,Cidade,Estado,Cep,DDD,Telefone,Atividade,EmailComercial,EmailCobranca,EmailNFe,ContatoComercial,ContatoCobranca,Emissao,Entrega,EntregaMaxima,PedidoDoCliente,Vendedor,AgenteDeVenda,AgenteDeVenda2,Instalador,Transportadora,FretePorConta,OperadorLeasing,Distribuidora,UsoDaMercadoria,Ent_MesmoEndereco,Ent_Cnpj,Ent_Endereco,Ent_Numero,Ent_Complemento,Ent_Bairro,Ent_Cidade,Ent_Estado,Ent_Cep,CondicaoDePagamentoFat,ValorFinanciado,TipoDeFinanciamento from Pedidos_Cab order by id", (error, results) => {
        if (error) {
...
//       const { id_original , NumeroDoPedido , Situacao , DataPrevista , DataDaOcorrencia , MensagemDeLog , Arquivo , Nome, Email, Cpf_Cnpj, ddi , ddd , Telefone, Cep, Cidade, Uf, Atividade , assunto, id_Segmento, id_Produto  , criacao , id_origem , mensagem , Produtos } = request.body
   ...
   let Ins3 = await pool.query
           (
'insert into pedidos_cab( ProcessoCRM , IndicadorDeInscricaoEstadual ,  InscricaoEstadual , Cnpj,Nome,Fantasia,Endereco,Numero,Complemento,Bairro,Cidade,Estado,Cep,DDD,Telefone,Atividade,EmailComercial,EmailCobranca,EmailNFe,ContatoComercial,ContatoCobranca,Emissao,Entrega,EntregaMaxima,PedidoDoCliente,Vendedor,AgenteDeVenda,AgenteDeVenda2,Instalador,Transportadora,FretePorConta,OperadorLeasing,Distribuidora,UsoDaMercadoria,Ent_MesmoEndereco,Ent_Cnpj,Ent_Endereco,Ent_Numero,Ent_Complemento,Ent_Bairro,Ent_Cidade,Ent_Estado,Ent_Cep,CondicaoDePagamentoFat,ValorFinanciado,TipoDeFinanciamento,Situacao_Proc ) '+
...
//            [ id_original , NumeroDoPedido , Situacao , DataPrevista , DataDaOcorrencia , MensagemDeLog , Arquivo , 0 , Nome, Email, Cpf_Cnpj, ddi , ddd , Telefone, Cep, Cidade, Uf, assunto, id_Segmento, id_Produto , criacao , id_origem , mensagem , '0' ]
              [ ProcessoCRM , IndicadorDeInscricaoEstadual , InscricaoEstadual , Cnpj,Nome,Fantasia,Endereco,Numero,Complemento,Bairro,Cidade,Estado,Cep,DDD,Telefone,Atividade,EmailComercial,EmailCobranca,EmailNFe,ContatoComercial,ContatoCobranca,Emissao,Entrega,EntregaMaxima,PedidoDoCliente,Vendedor,AgenteDeVenda,AgenteDeVenda2,Instalador,Transportadora,FretePorConta,OperadorLeasing,Distribuidora,UsoDaMercadoria,Ent_MesmoEndereco,Ent_Cnpj,Ent_Endereco,Ent_Numero,Ent_Complemento,Ent_Bairro,Ent_Cidade,Ent_Estado,Ent_Cep,CondicaoDePagamentoFat,ValorFinanciado,TipoDeFinanciamento , '0' ]
           )
        if (Ins3.rows[0] )
           {
...
const getPedidos_Phoenix = async (request, response) => 
{
    ...
    try
   {
       ...
       for (let zFor = 0 ; zFor < Get3Json.rows.length ; zFor++ )
       {
        ...
        for (let zFor = 0 ; zFor < Get5Json.rows.length ; zFor++ )
        {
            Get6 = await pool.query("Select Codigo,Valor Pedidos_Caracteristicas Where id = $1 and id_prod = xxx order by Sequencial",[ Get5Json.rows[zFor].id ] )
